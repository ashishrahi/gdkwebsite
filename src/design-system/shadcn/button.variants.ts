...
export const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-xl border border-transparent bg-clip-padding text-sm font-semibold tracking-[var(--ds-type-label-letter-spacing)] whitespace-nowrap transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-ds-out outline-none select-none focus-visible:border-ring focus-visible:ring-4 focus-visible:ring-ring/20 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-4 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "bg-[var(--brand-accent)] text-primary-foreground shadow-[var(--ds-shadow-button-primary)] hover:-translate-y-0.5 hover:bg-[color:var(--brand-accent-hover)] hover:shadow-[var(--ds-shadow-button-primary-strong)]",
        outline:
          "border-[color:color-mix(in_srgb,var(--brand-blue-500)_36%,var(--border))] bg-background/90 text-foreground shadow-[var(--ds-shadow-xs)] hover:-translate-y-0.5 hover:border-[color:var(--brand-blue-500)] hover:bg-accent hover:text-accent-foreground hover:shadow-[var(--ds-shadow-sm)] aria-expanded:bg-accent aria-expanded:text-accent-foreground dark:border-input dark:bg-input/30 dark:hover:bg-input/50",
        secondary:
      ...
      size: {
        ...
        sm: "h-10 gap-1.5 rounded-lg px-4 text-[0.8125rem] in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3 [&_svg:not([class*='size-'])]:size-3.5",
        lg: "h-12 gap-2 rounded-xl px-7 has-data-[icon=inline-end]:pr-6 has-data-[icon=inline-start]:pl-6",
        icon: "size-11",
        "icon-xs":
